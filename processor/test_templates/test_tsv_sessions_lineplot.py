...
Feed hand-built TSVs to render() and check two things:
  1. valid data          -> a PNG is written
  2. each kind of bad data -> RuntimeError is raised (in production this raise
                              is what falls the processor through to the agent loop)
...
########## good path and genearlly good input
### 1. 
# produce a PNG
...
#### 2.
# test that a non-TSV input is rejected
...
#### 3.
# test if error is raised when there are too few plottable sessions (a line
...
# test if error is raised when there are too many sessions (> MAX_SESSIONS):
...
########### bad data
#### 1. 
# test if error is raised when no appropriate columns is given
...
#### 2. 
# test if error is raised when the input tsv is empty 
...
#### 3. 
# test if error is rasied when the session id column is missing
def test_render_raises_when_no_session_column(tmp_path):
    src = _write_tsv(tmp_path / "sessions.tsv", "subject_age\tnotes\n10.5\tfoo\n")
    with pytest.raises(RuntimeError):
        template.render(src, str(tmp_path / "figure.png"))

# test if error is raised when the age column is missing
...
#### 4.
# test if error is raised when all session id values are empty
def test_render_raises_when_all_session_id_empty(tmp_path):
    src = _write_tsv(tmp_path / "sessions.tsv", "session_id\tsubject_age\n\t1.5\n\t4.7\n")
    with pytest.raises(RuntimeError):
        template.render(src, str(tmp_path / "figure.png"))

# test if error is raised when all age values are empty
...
#### 5. 
# test if error is rasied when session id is non-unique
...
# test if error is raised when age is non-unique
...
#### 6.
# test if error is raised when all ages are non-numeric
...
########### plotting check
#### 1.
# test that the line is plotted in ascending age order, no matter the row order in the file. 
...
#### 2.
# test that a row with a MISSING session id (but valid age) is dropped, not
